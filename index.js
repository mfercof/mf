       ...
       case 'twt':
           tweetdl(value)
               .then(data => {
                    ...
                    let hasil = `✄1�71ￄ1�77 PERFECTO! Eliga su metodo de descarga!\nKlik link dibawah🗡️\n\nSize: ${size}\n\nLink: ${link}`
                    client.sendMessage(id, '[ESPERE UN MOMENTO...', MessageType.text)
                    client.sendMessage(id, hasil ,MessageType.text)
               })
               .catch(err => {
       ...
       case 'wiki':
           wiki(value)
               .then(data => {
                    ...
                    let hasil = `📝De acuerdo a Wikipedia:\n\n${res}`
                    client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
       ...
       case 'sholat':
           jsholat(value)
               .then(data => {
                   ...
                   let hasil = `Momentos de oracion en *${value}* hoje é\n\n⚡Imsak : ${Imsyak}\n⚡Subuh : ${Subuh} WIB\n⚡Dzuhur : ${Dzuhur}WIB\n⚡Asr: ${Ashar} WIB\n⚡Maghrib : ${Maghrib}\n⚡Isya : ${Isya} WIB\n⚡Tengah malam : ${Dhuha} WIB`
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
       ...
       case 'covid':
           corona()
               .then(data => {
                   ...
                   let hasil = `📌ÚLTIMOS DATOS DE DISTRITO COVID-19 EN INDONÉSIA\n\n📍Positivo ==> ${positif} \n📍Curado ==> ${sembuh} \n📍Morreu ==> ${meninggal}`
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
...
case 'puisi2':
 ...
 case 'infogempa':
   ...
   case 'chord':
           chord(value)
               .then(data => {
                   const { result } = data
                   let hasil = `Aqui estan los acordes de música *${value}* querida ♥️\n\n  _${result}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
   ...
   case 'kbbi':
           kbbi(value)
               .then(data => {
                   const { result } = data
                   let hasil = `*${value}* de acuerdo a KBBI ️\n\n  _${result}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
 ...
 zodiak(value)
               .then(data => {
                   ...
                   let hasil = `*Nacimiento* : _${lahir}_ n\n *Aniversário* : _${ultah}_ \n *Edad* : _${usia}_:\n *Zodíaco* : _${zodiak}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
 ...
 case 'wikien':
           wikien(value)
               .then(data => {
                   const { result } = data
                   let hasil = `*♻️De acuerdo a Wikipedia🗿:* \n\n  _${result}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
  ...
  case 'cektanggal':
           cektanggal(value)
               .then(data => {
                   ...
                   let hasil = `Segun la fecha ${value} adalah\n\n *Fecha* : _${tanggal}_ \n *Informacion* : _${keterangan}_  `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
   ...
   case 'bitly':
           bitly(value)
               .then(data => {
                   const { result } = data
                   let hasil = `Aquí hermanita ya terminó☣️ :) \n\n${result} `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
      ...
      case 'stickergif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    ...
                    client.reply(from, '[WAIT] Sedang di proses⏄1�71ￄ1�77 puede tardar ± 1 min!', id)
                    ...
                    client.reply(from, '[❗] Enviar video con leyenda *!stickerGif* max 10 sec!', id)
                )
            }
            break     
     case 'sticker':
